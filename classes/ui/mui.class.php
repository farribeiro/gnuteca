...
//$this->manager->trace(print_r($m,true));
	...
	{
            if ( MUtil::getBooleanValue( $this->manager->getConf('options.absolute_url')) )
            {
                $url = $this->manager->getAbsoluteURL("images/modules/{$module}/{$name}");
            }
            else
            {
    ...
    {
        $link = "javascript:miolo.getWindow('{$winId}').open();";
        return $link;
    }
    ...
    public function closeWindow($winId = '')
    {
        ...
        $id = ($winId != '' ? $winId : $MIOLO->page->winid != '' ? $MIOLO->page->winid : '');
        $link = $id != '' ? "javascript:miolo.getWindow('{$id}').close();" : '';
        return $link;
    }
    ...
    public function getAjax($action)
    {
        ...
        $link = "miolo.doAjax('{$eventTokens[0]}','{$eventTokens[1]}','$formId')";
        return $link;
    }
