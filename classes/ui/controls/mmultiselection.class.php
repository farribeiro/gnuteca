...
class MMultiSelectionField extends MListControl
{
    ...
    public function generateInner()
    {
        ...
        $mtsName = "mts_{$formId}_{$this->name}";
        $this->page->addJsCode("{$mtsName} = new Miolo.MultiSelection('{$mtsName}');"); 

        $this->page->onSubmit("{$mtsName}.onSubmit('{$formId}','{$this->name}')");
        ...
        $field = new MMultiSelection("{$mtsName}[]", array(), $labelS, $content, '', '', $this->numRows);

        $field->_AddStyle('width', "{$this->colWidth}px");
        $field->addAttribute('onKeyDown', "return miolo.multiSelection.onKeyDown(event,$numFields);");
        $field->addAttribute('onChange', "{$mtsName}.onSelect($numFields)");
        ...
        {
            $f->setId("{$mtsName}_options{$n}");
            $f->setName("{$mtsName}_options{$n}");
            $f->setClass('combo');
        ...
        $b = $buttons[] = new MButton("{$this->name}_remove", 'Remover',
                                      "{$mtsName}.remove($numFields)");
        $b->setClass('button');

        if ($this->shownav)
        {
            $b = $buttons[] = new MButton("{$this->name}_up", '/\\',
                                          "{$mtsName}.moveUp($numFields)");
            $b->setClass('button');
