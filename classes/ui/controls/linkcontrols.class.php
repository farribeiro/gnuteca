...
//        $this->page->addScript('m_linkbutton.js');
    ...
    public function generateLink()
    {
        ...
        else if ( substr($actionUpper, 0, 7) == 'HTTP://' || substr($actionUpper, 0, 8) == 'HTTPS://' )
        {
            if ( count($this->eventHandlers) )
            {
                $param   = $this->eventHandlers['click']['param'];
...
class MImageLink extends MLink
{
    ...
    {
        $this->image->addAttribute('alt', $this->getAttribute('alt'));
        $this->image->addAttribute('title', $this->getAttribute('title'));
