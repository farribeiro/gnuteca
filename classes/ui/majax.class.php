  ...
  class MAjax extends MService {
    ...
    */
    public function __construct() {
      ...
      //Caracteres especiais que não devem ser perdidos na codificação e decodificação
      $this->special_char->char   = array( 'σ', 'α', 'β', 'χ', 'δ', 'φ', 'γ', 'η', 'κ', 'λ', 'ν', 'μ', 'ο', 'π', 'θ', 'ρ', 'τ', 'υ', 'ϖ', 'ω', 'ξ', 'ψ', 'ζ',
                                           '≅', 'Δ', 'Φ', 'Γ', 'Λ', 'Π', 'Θ', 'Σ', 'Ω', 'Ξ', 'Ψ', '≠', '∑', '≤', '≥', '®');
      
      foreach ($this->special_char->char as $charKey => $char)
      {
          $this->special_char->string[$charKey] = '&specialChar' .  $charKey . '_';
      }
...
*/
    ...
    * @return   string
    */
    public function setResponseControls($controls,$elements='') 
    {
      MUtil::SetIfNull($this->response,new MDummy());
    ...
    public function setResponse($controls,$elements='',$scripts=array()) 
    {
...
//      return $GLOBALS['__cpaint_json']->stringify($JSON_node);
    ...
    */
    public function encode($data, $encoding) {
      if ( preg_match('/[' . implode('', $this->special_char->char) . ']/', $data) ) {
        $data = str_replace($this->special_char->char,$this->special_char->string, $data);
        $specialChar = true;
      }
      // convert string
...
//      $return_value = str_replace(chr(0x3e), '\u003e', $return_value);
      ...
      if ( $specialChar ) {
        $return_value = str_replace($this->special_char->string, $this->special_char->char, $return_value);
      }
