...
class Cpdf
{
    ...
    */
    ...
    public function cpdf($pageSize = array(0, 0, 612, 792))
    {
        $this->newDocument($pageSize);

        $this->setFontsPath('./fonts');
        ...
        switch ($action)
            ...
            case 'new':
                ...
                switch ($options['type'])
                    {
                    case 'XYZ':
                    case 'FitR': $tmp = ' ' . $options['p3'] . $tmp;

                    case 'FitH':
                    case 'FitV':
                    case 'FitBH':
                    case 'FitBV': $tmp = ' ' . $options['p1'] . ' ' . $options['p2'] . $tmp;
                    ...
                    if ((isset($o['info']['fonts']) && count($o['info']['fonts'])) || isset($o['info']['procset']))
                    {
                        ...
                        if (isset($o['info']['mediaBox']))
                        {
                            ...
                            $res .= "\n/MediaBox [" . __sprintf('%.3f',
                                                                $tmp[0]) . ' ' . __sprintf('%.3f',
                                                                                           $tmp[1]) . ' '
                                        . __sprintf('%.3f', $tmp[2]) . ' ' . __sprintf('%.3f', $tmp[3]) . ']';
                        }
                    }
        ...
        switch ($action)
            ...
            case 'new':
                ...
                $this->objects[$id] = array
                    ...
                    't' => 'info',
                    'info' =>
                        array('Creator' => 'R and OS php pdf writer, http://www.ros.co.nz', 'CreationDate' => $date)
                    );
        ...
        switch ($action)
            ...
            case 'new':
                ...
                $this->objects[$id] = array
                    (
                    't' => 'page',
                    'info' => array('parent' => $this->currentNode, 'pageNum' => $this->numPages)
                    );
            ...
            case 'new':
                ...
                else if ($options['type'] == 'png')
                {
                    ...
                    $this->objects[$id]['info']['DecodeParms']
                        = '<< /Predictor 15 /Colors ' . $options['ncolor'] . ' /Columns ' . $options['iw']
        ...
        switch ($action)
            ...
            case 'new':
                ...
                $pad = chr(0x28) . chr(0xBF) . chr(0x4E) . chr(0x5E) . chr(0x4E) . chr(0x75) . chr(0x8A) . chr(0x41)
                           . chr(0x64)
                           . chr(0x00) . chr(0x4E) . chr(0x56) . chr(0xFF) . chr(0xFA) . chr(0x01) . chr(0x08) . chr(
                                                                                                                     0x2E)
                           . chr(0x2E) . chr(0x00) . chr(0xB6) . chr(0xD0) . chr(0x68) . chr(0x3E) . chr(0x80) . chr(
                                                                                                                     0x2F)
                           . chr(0x0C) . chr(0xA9) . chr(0xFE) . chr(0x64) . chr(0x53) . chr(0x69) . chr(0x7A);
                ...
                $tmp = $this->md5_16($user . $ovalue . chr($options['p']) . chr(255) . chr(255) . chr(255)
                                         . $this->fileIdentifier);
                $ukey = substr($tmp, 0, 5);
    ...
    */
    public function encryptInit($id)
    {
        ...
        $tmp .= chr(hexdec(substr($hex, 4, 2))) . chr(hexdec(substr($hex, 2, 2))) . chr(hexdec(substr($hex, 0,
                                                                                                      2))) . chr(0)
                    . chr(0);
        $key = $this->md5_16($tmp);
    ...
    */
    ...
    {
        if (strpos($fontName, $this->fonts_path) === false)
            $fontName = $this->fonts_path . '/' . $fontName;
        //echo $fontName;
    ...
    */
    public function setCurrentFont()
    {
        if (strlen($this->currentBaseFont) == 0)
        {
            ...
            $this->selectFont($this->fonts_path . '/Helvetica.afm');
        }

        $cf = substr($this->currentBaseFont, strrpos($this->currentBaseFont, '/') + 1);

        if (strlen($this->currentTextState) && isset($this->fontFamilies[$cf]) && isset(
                                                                                      $this->fontFamilies[$cf][$this->currentTextState]))
        {
            // then we are in some state or another
            // and this font has a family, and the current setting exists within it
            // select the font, then return it
            $nf = substr($this->currentBaseFont, 0,
                         strrpos($this->currentBaseFont, '/') + 1) . $this->fontFamilies[$cf][$this->currentTextState];
    ...
    */
    ...
    {
        if ($r >= 0 && ($force || $r != $this->currentColour['r'] || $g != $this->currentColour['g']
    ...
    */
    ...
    {
        if ($r >= 0 && ($force || $r != $this->currentStrokeColour['r'] || $g != $this->currentStrokeColour['g']
    ...
    */
    public function curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3)
    {
        ...
        $this->objects[$this->currentContents]['c'] .= "\n" . __sprintf('%.3f',
                                                                        $x0) . ' ' . __sprintf('%.3f',
                                                                                               $y0) . ' m '
                                                           . __sprintf('%.3f', $x1) . ' ' . __sprintf('%.3f', $y1);
        $this->objects[$this->currentContents]['c'] .= ' ' . __sprintf('%.3f',
                                                                       $x2) . ' ' . __sprintf('%.3f',
                                                                                              $y2) . ' '
                                                           . __sprintf('%.3f',
                                                                       $x3) . ' ' . __sprintf('%.3f', $y3) . ' c S';
    }
    ...
    */
    public function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart = 0, $afinish = 360, $close = 1, $fill = 0)
    {
        ...
        if ($angle != 0)
        {
            ...
            $tmp .= __sprintf('%.3f',
                              cos($a)) . ' ' . __sprintf('%.3f',
                                                         (-1.0 * sin($a))) . ' ' . __sprintf('%.3f',
                                                                                             sin($a)) . ' '
                        . __sprintf('%.3f', cos($a)) . ' ';
            $tmp .= __sprintf('%.3f', $x0) . ' ' . __sprintf('%.3f', $y0) . ' cm';
    ...
    */
    public function newPage($insert = 0, $id = 0, $pos = 'after')
    {
        ...
        {
            $this->setStrokeColor($this->currentStrokeColour['r'], $this->currentStrokeColour['g'],
                                  $this->currentStrokeColour['b'], 1);
        }
    ...
    */
    public function getFontHeight($size)
    {
        if (!$this->numFonts)
        {
            $this->selectFont($this->fonts_path . '/Helvetica');
        }
                        ...
                        case 'i':
                            ...
                            if ($text[$j] == '>')
                            {
                                ...
                                if ($p !== false)
                                {
                                    ...
                                    $this->currentTextState = substr($this->currentTextState, 0,
                                                                     $p) . substr($this->currentTextState, $p + 1);
                                }
                            ...
                            if ($k !== false && $text[$j] == ':')
                            {
                                ...
                                else
                                {
                                    ...
                                    if ($final)
                                    {
                                        ...
                                        $tmp = $this->PRVTgetTextPosition(
                                                   $x, $y, $angle, $size, $wordSpaceAdjust, substr($text, 0, $i));
                                        $info = array
                                            (
                                            'x' => $tmp[0],
                                            'y' => $tmp[1],
    ...
    */
    public function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0)
    {
            ...
            $tmp .= __sprintf('%.3f',
                              cos($a)) . ' ' . __sprintf('%.3f',
                                                         (-1.0 * sin($a))) . ' ' . __sprintf('%.3f',
                                                                                             sin($a)) . ' '
                        . __sprintf('%.3f', cos($a)) . ' ';
            $tmp .= __sprintf('%.3f', $x) . ' ' . __sprintf('%.3f', $y) . ' Tm';
            $this->objects[$this->currentContents]['c'] .= $tmp;
        }
        ...
        if ($wordSpaceAdjust != 0 || $wordSpaceAdjust != $this->wordSpaceAdjust)
        {
            ...
            $this->objects[$this->currentContents]['c'] .= ' ' . __sprintf('%.3f', $wordSpaceAdjust) . ' Tw';
        }
        ...
        for ($i = 0; $i < $len; $i++)
        {
            ...
            if ($directive)
            {
                ...
                if ($i > $start)
                {
                    ...
                    $this->objects[$this->currentContents]['c']
                        .= ' /F' . $this->currentFontNum . ' ' . __sprintf('%.1f', $size) . ' Tf ';
                ...
                else
                {
                    ...
                    $xp = $x;
                    $yp = $y;
                    $directive = $this->PRVTcheckTextDirective1(
                        ...
                        $tmp .= __sprintf('%.3f',
                                          cos($a)) . ' ' . __sprintf('%.3f',
                                                                     (-1.0 * sin($a))) . ' ' . __sprintf('%.3f',
                                                                                                         sin($a)) . ' '
                                    . __sprintf('%.3f', cos($a)) . ' ';
                        $tmp .= __sprintf('%.3f', $xp) . ' ' . __sprintf('%.3f', $yp) . ' Tm';
                        $this->objects[$this->currentContents]['c'] .= $tmp;
                    }
        ...
        if ($start < $len)
        {
            ...
            $this->objects[$this->currentContents]['c'] .= ' /F' . $this->currentFontNum . ' ' . __sprintf('%.1f',
                                                                                                           $size)
                                                               . ' Tf ';
            $this->objects[$this->currentContents]['c'] .= ' (' . $this->filterText($part) . ') Tj';
        }
    ...
    */
    public function addJpegImage_common(&$data, $x, $y, $w = 0, $h = 0, $imageWidth, $imageHeight, $channels = 3)
    {
        ...
        $label = 'I' . $im;
        $this->numObj++;
        $this->o_image($this->numObj,
                       'new',
                       array('label' => $label, 'data' => $data, 'iw' => $imageWidth, 'ih' => $imageHeight,
                             'channels' => $channels));
    ...
    */
    ...
    public function transaction($action)
    {
        switch ($action)
            ...
            case 'commit':
                if (is_object($this->checkpoint) && isset($this->checkpoint->checkpoint))
                {
                    $tmp = $this->checkpoint->checkpoint;
                    $this->checkpoint = $tmp;
                    unset($tmp);
    ...
    public function setFontsPath($path)
    {
