...
/* code modified from ColdFusion's cfdump code */
    public function dBug_toggleRow(source) {
    ...
    public function dBug_toggleSource(source) {
    ...
    public function dBug_toggleTarget(target,switchToState) {
        target.style.display=(switchToState=='open') ? '' : 'none';
    }

    public function dBug_toggleTable(source) {
        public switchToState=dBug_toggleSource(source);
        if(document.all) {
            public table=source.parentElement.parentElement;
            ...
            public table=source.parentNode.parentNode;
...
class dBug
{
    ...
    public function dBug($var, $forceType = "")
    {
        ...
        if (in_array ( $forceType, $arrAccept ))
            $this->{"varIs" . ucfirst ( $forceType )} ( $var ); else
            $this->checkType ( $var );
    }
    ...
    public function xmlStartElement($parser, $name, $attribs)
    {
        ...
        if (count ( $attribs ) > 0)
            $this->xmlSData[$this->xmlCount] .= '$this->varIsArray($this->xmlAttrib[' . $this->xmlCount . ']);'; else
