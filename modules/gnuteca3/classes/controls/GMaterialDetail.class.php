...
class GMaterialDetail extends MDiv
{
    ...
    function getKardexFields()
    {
        ...
        if ( $r )
        {
            ...
            foreach ($r as $c)
            {
                ...
                if ( is_array($exemplarys) )
                {
                    ...
                    foreach ( $exemplarys as $k=> $exem )
                    {
                        $types[$exem->materialPhysicalTypeId] = $exem->materialPhysicalTypeId; //obtém o tipo fisico do exemplar, não repete o tipo
                    }
                }
                
                //ordena os tipos de materiais
                ksort($types);
                //carrega as imagens para cada fascículo (por tipo de material)
                if ( is_array($types) )
                {
                    foreach ( $types as $key => $type )
                    {
                        $businessFile->fileName = $type . '.';
                        $file = $businessFile->searchFile(true);

                        if ( $file[0] )
                        {
                            $image = new MImage(null, null, $file[0]->mioloLink);
                            $images[$_controlNumber] .= $image->generate();
                        }
                    }
                }

                list($fieldId, $subFieldId) = explode('.', MARC_EXEMPLARY_LIBRARY_UNIT_ID_TAG);
                $libraryUnits = $businessMaterial->getContent($_controlNumber, $fieldId, $subFieldId, null, null, TRUE);

                if ($libraryUnits)
                {
                    foreach ($libraryUnits as $key => $val)
                    {
                        $libraryUnitId = $val[0];
                        
                        if (!in_array($_controlNumber, $referenciados[$libraryUnitId]))
                        {
                            $referenciados[$libraryUnitId][] = $_controlNumber; 
                        }
                    }
                }
            }
        ...
        foreach ($kardexUnits as $i => $content)
        {
            ...
            foreach ($referenciados[$i] as $line=>$controlNumber)
            {
                list($f,$s) = explode(".", MARC_PERIODIC_INFORMATIONS);

                if(ereg ("(v\. [^,]*,)?( ?n\. [^,]*,)?( ?[^ ]* )?([0-9]{4})", $businessMaterial->getContent($controlNumber, $f, $s), $regs))
                {
                    $fasciculo[] = $regs;
                    ...
                    $numero = trim(str_replace(',', '', str_replace('n.', '', $regs[2]))); //obtém o número
                    ...
                    $link = new MLink('', $images[$controlNumber]. ' ' .$label, "javascript:".GUtil::getAjax("openMaterialDetail", $controlNumber));
                    $detalhamentoOrder[$ano][$volume][$index] = $numero;
            ...
            {
                $nOrder = $detalhamentoOrder[$ano]; //array para ordenamento dos números
                ...
                foreach($volumesOrdered as $l=> $volume)
                {
                    ...
                    array_multisort($nOrder[$volume], $n1[$volume]);
                    $sum = ($colN+$colV+1);
    ...
    public function getReserveFields()
    {
        ...
        $position = 1; //posição da reserva

        foreach ($reserves as $line => $reserve)
        {
        	//se reserva for atendida ou comunicada a posição é sempre 1
            ...
            $lineData[] = $this->bold($bold, $position . 'º') ;
            ...
            //se trocar o código da reserva acrescenta a posição
            if ( ( $reserve->reserveId != $reserves[$line+1]->reserveId ))
            {
                $position ++;
            }
     ...
     * @return unknown_type
     */
    public function getMainTabFields( $args )
    {
    	$args = ( Object ) $_REQUEST;
