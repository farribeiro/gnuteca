...
class GRequestChangeExemplaryStatus
{
    public function mainFields()
    {
        ...
        if ($args->reserveLibraryId)
        {
            ...
            if ( MUtil::getBooleanValue( REQUEST_CHANGE_EXEMPLARY_STATUS_BY_SEMESTER ) )
            {
                ...
                // PEGA OS STARTING PARA VER QUAL PERIODO SE ENCAIXA
                foreach ($periodos as $x => $period)
                {
                    ...
                    if(preg_match('/Starting:[0-9]{2}\/[0-9]{2}/', $period, $starting))
                    {
                        $starting = str_replace('Starting:', '', $starting[0]);
                        $dateDb = new GDate("$starting/". date('Y'));
                        $startingDate[$periodDescription] = $dateDb->getDate(GDate::MASK_TIMESTAMP_USER);
                    }
                }

                $mostRecentPeriod = null;

                foreach ($startingDate as $periodDescription => $dateSt)
                {
                    $dateSt =  new GDate ($dateSt);
                    $subtractTime = $today->subtractDate($dateSt);

                    if ($subtractTime <= -1) //Volta para ano anterior
                    {
                        $dateSt->addYear(-1);
                        $subtractTime = $today->subtractDate($dateSt);
                    }
                    if (!$mostRecentPeriod || ($subtractTime < $mostRecentPeriod))
                    {
                        $mostRecentPeriod = $subtractTime;
                        foreach ($periodos as $x => $period)
                        {
                            if(ereg("$periodDescription=", $period))
                            {
                                $starting = $period;
                            }
                        }
                ...
                if(preg_match('/StartDate:[0-9]{2}\/[0-9]{2}/', $starting, $sDate))
                {
                    $sDate = str_replace("StartDate:", "", $sDate[0]);
                    $sDate = new GDate("$sDate/". date("Y"));
                }

                if(preg_match('/EndDate:[0-9]{2}\/[0-9]{2}/', $starting, $fDate))
                {
                    $fDate = str_replace("EndDate:", "", $fDate[0]);
                    $fDate = new GDate("$fDate/". date("Y"));
                }

                if ($sDate && $fDate) //verificacao preventiva
                {
                    if($dateDb->compare($sDate, '>'))
                    {
                        $sDate->addYear(1);
                    }

                    while($sDate->compare($fDate, ">"))
                    {
                        $fDate->addYear(1);
                    }

                    $sDateValue  = $sDate->getDate(GDate::MASK_DATE_DB);
                    $fDateValue  = $fDate->getDate(GDate::MASK_DATE_DB);
                }
            ...
            if ( $datesReadOnly )
            {
                $dBegin = new MTextField('date', $sDate->getDate(GDate::MASK_DATE_USER), _M('Data', $module), FIELD_DATE_SIZE );
                $dFinal= new MTextField('finalDate', $fDate->getDate(GDate::MASK_DATE_USER) , _M('Data final', $module), FIELD_DATE_SIZE );
                $dBegin->setReadOnly(true);
                $dFinal->setReadOnly(true);
            }
            else
            {
                $dBegin   = new MCalendarField('date', GDate::now()->getDate(GDate::MASK_DATE_DB), _M('Data', $module) );
                $dFinal   = new MCalendarField('finalDate', GDate::now()->getDate(GDate::MASK_DATE_DB),_M('Data final', $module));
            }
        ...
        if ( $busOperationRequestChangeExemplaryStatus->checkAccess() )
        {
            foreach ($args->listItemNumber as $cont)
            {
                ...
                foreach ($it as $itenN)
                {
                    ...
                    if ( ($exemplaryStatusId != DEFAULT_EXEMPLARY_STATUS_DESAPARECIDO) && ($exemplaryStatusId != DEFAULT_EXEMPLARY_STATUS_DANIFICADO) && ($exemplaryStatusId != DEFAULT_EXEMPLARY_STATUS_DESCARTADO) )
                    {
            ...
            catch (Exception $exc)
            {
                ...
                $box= MMessage::getStaticMessage( MMessage::MSG_DIV_ID, $exc->getMessage() , MMessage::TYPE_WARNING );
                $box->addAttribute('alt',$exc->getMessage());
                $box->addAttribute('title',$exc->getMessage());
