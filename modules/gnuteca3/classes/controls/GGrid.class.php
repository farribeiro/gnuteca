...
class GGrid extends MGrid
{
    ...
    public function __construct($data, $columns, $href, $pageLength = null, $index = null, $name = null, $useSelecteds = null, $useNavigator = null)
    {
        ...
        $empty[] = new MImage('emptyImg', null, Gutil::getImageTheme('info.png'));
    ...
    public function addActionIcon($alt, $icon, $href, $index = 0, $imageName = 'edit' )
    {
    ...
    public function generateActions(&$tbl)
    {
        ...
        {
            $tbl->cell[$i][$inicio] = new Div(null, implode(' ',$tmp), 'gridActions');
        }
    ...
    function generate()
    {
        ...
        if (is_array($data) && ($comma) && $this->useCSV)
        {
            if ($event == 'generateGridCSV')
            {
                ...
                if (is_array($columns))
                {
                   foreach ($columns as $line => $column)
                   {
                        ...
                        if ($column->visible)
                        {
                            $title = str_replace($comma, $other, $column->title );
                            $csv .= $title . $comma;
                        }
                   }
                }
                   ...
                   if (is_array($info))
                   {
                        foreach ( $info as $l => $i )
                        {
                            ...
                            if ( $column->visible )
                            {
                                ...
                                $tempData = str_replace( $comma, $other, $i );
                ...
                BusinessGnuteca3BusFile::openDownload('grid',$this->name.'.csv', $csv);
            }
            else if ($event == 'generateGridPdf')
            {
                ...
                BusinessGnuteca3BusFile::openDownload('grid',$this->name.'.pdf', $pdf->Output(null, 'S'));
            }
