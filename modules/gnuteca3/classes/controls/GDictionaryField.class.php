...
class GDictionaryField extends MLookupTextField
{
    ...
    public function generate()
    {
        ...
        if ( !$readonly )
        {
            $this->addAttribute('onkeydown', "return onkeyUpDictionary(event, this,'{$this->name}','{$this->related}', '{$this->item}', '{$this->filter}');" ); //ajax
    ...
    public static function onkeyUpDictionary($args)
    {
        ...
        //trata os dados transformando em objetos, com links
        if ( $result )
        {
            $cont = 0;
            ...
            {
                $valueScaped = str_replace("'", "\'", $info[0] ); //escapa string
                $onclick = 'dojo.byId(\''.$div.'\').style.display=\'none\'; dojo.byId(\''.$name.'\').value = \''.$valueScaped.'\'';
                $result[$line] = "<div id='{$name}Item{$cont}' onclick=\"$onclick\">$info[0]</div>";
                $cont ++;
            }
        }
