...
class GRepetitiveField extends MBaseGroup
{
   ...
   /**
    ...
    public static function fileUpload( $repetitive )
    {
        ...
        {
            throw new Exception ( _M('É necessário selecionar um arquivo para efetuar o envio.', 'gnuteca3' ) );
        }
        ...
        {
            unlink($uploadedFile->tmp_name); //remove arquivo
            throw new Exception( _M('Excede o limite permitido.','gnuteca3') );
        }
    }
...
function autoEditAction( $args )
{
	...
	if ( is_array( $fieldNames) )
	{
		...
		foreach ( $fieldNames as $line => $info )
		{
            //define o foco no primeiro campo, não esta controlando se for campo escondido
            if ( $x == 0 )
            {
                GForm::jsSetFocus($info);
            }
            
            //seta o foco nos campos
            if ( !$line || $line == 0)
            {
                $temp .= "gnuteca.setFocus('{$info}');";
            }

            $x++;

            //engana o addslashes(stripslashes($value)), substituindo / por //
            $value = str_replace("\\", "\\\\", $values->$info );
            //ajeita conteúdo para javascript
            $value = addslashes(stripslashes($value));
			$value = str_replace("\n", '\n', $value); 
			$value = str_replace("\r", '\r', $value);
            //adiciona barra extra para ser retirada pelo javascript
            //isso é necessário para poder editar conteúdo html/xml
