...
class GReserveWeb
{
	...
	public function selectReserveLibraryUnit($args)
	{
        ...
        //verifica se precisa montar relação de item/Volume/Tomo
	    if ( is_array( $filter ) )
	    {
	    ...
	    //caso não precise de relação de tipo de material/volume/tomo passa direto para a finalização da reserva
        if ( (!$temVolumeOuTomo && !$temMaterial )  || $args->initialStatusConfirmed )
	    {
    ...
    */
    ...
    public function btn_reserveConfirm_click( $args )
	{
        ...
        //Se estiver acessando do módulo Circulação de material a reserva é local, pois o usuário esta no balcão
		...
		if ($args->doFilter || is_array( $args->reserveFilter) )
		{
			...
			{
				$i = 1;
			    foreach ( $args->reserveFilter as $line => $info)
				{
					$filter = explode( ',' , $args->reserveFilter[$line] );
