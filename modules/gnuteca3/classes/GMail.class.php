...
class GMail extends PHPMailer
{
    ...
    function __construct()
    {
        ...
        $this->setHost(EMAIL_SMTP);
        $this->setPort(EMAIL_PORT);
        $this->setFrom(EMAIL_FROM);
        $this->setFromName(EMAIL_FROM_NAME);
        $this->setAuthenticate(EMAIL_AUTHENTICATE);
        $this->setUser(EMAIL_USER);
        $this->setPassword(EMAIL_PASSWORD);
        $this->setContentType(EMAIL_CONTENT_TYPE);
    ...
    {
        //concatena o prefixo
        $this->Subject = EMAIL_SUBJECT_PREFIX . ' ' . $subject;
        //tira espaços extras
    ...
    public function send()
    {
        ...
        if ( MUtil::getBooleanValue( EMAIL_TESTING ) && strlen(EMAIL_TEST_RECEIVE))
        {
            $this->setAddress(EMAIL_TEST_RECEIVE);
        }
    ...
    {
    	return utf8_encode( $this->ErrorInfo );
    }
