...
class GSendMail
{
		...
		foreach ($libraryItems as $libraryUnitId => $exemplaryes)
		{
			...
			switch ($type)
			{
				...
				case 'approve'      :
					...
					$action     = _M('aprovado',            $this->module);
					break;

				case 'disapprove'   :
					$content    = $this->busLibraryUnitConfig->getValueLibraryUnitConfig($libraryUnitIdRequest, 'EMAIL_LOANBETWEENLIBRARY_CONFIRMLOAN_CONTENT');
					$status     = _M('Reprovar material', $this->module);
					$action     = _M('não aprovado',        $this->module);
					break;
			}
    ...
    public function sendMailToAdminResultOfReserveAnswered($result, $libraryUnitId)
    {
        ...
        if ( count($result) > 0 )
        {
            ...
            (
                ...
                _M("Mensagem de devolução",    $this->module),
	 ...
	 * @return boolean
	 */
	public function sendMailToUserInformingReserveCancel($reserveId)
	{
		...
		$busMaterial    = $this->MIOLO->getBusiness($this->module, 'BusMaterial');
		$busLibraryUnit = $this->MIOLO->getBusiness($this->module, 'BusLibraryUnit');
		$subject        = $this->busLibraryUnitConfig->getValueLibraryUnitConfig($reserve->libraryUnitId, 'EMAIL_CANCEL_RESERVE_COMUNICA_SOLICITANTE_SUBJECT');
		$content        = $this->busLibraryUnitConfig->getValueLibraryUnitConfig($reserve->libraryUnitId, 'EMAIL_CANCEL_RESERVE_COMUNICA_SOLICITANTE_CONTENT');
