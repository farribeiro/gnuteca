    ...
    public function callFunction($function, $content)
    {
        ...
        switch (strtolower($f->name))
        {
            ...
            case 'cut':
                if ($f->parameter[1])
                {
                    $content = substr($content, $f->parameter[0], $f->parameter[1]);
                }
                else
                {
                    $content = substr($content, $f->parameter[0]);
                }
                break;
            ...
            case 'pad':
                ...
                if(strlen($content) > $f->parameter[0])
                {
                    $content = substr($content,0, $f->parameter[0]-3)."...";
                }
            ...
            case 'compare' :
                ...
                if(ereg("\([0-9]{1,20} [0-9]{3}\.[a-z0-9]{1}\)", $f->parameter[0]))
                {
                    ...
                    $f->parameter[0] = $this->busMaterial->getContentTag($controlNumber, $tag);
                }
                if(ereg("\([0-9]{1,20} [0-9]{3}\.[a-z0-9]{1}\)", $f->parameter[2]))
                {
                    ...
                    $f->parameter[2] = $this->busMaterial->getContentTag($controlNumber, $tag);
                }
            ...
            case 'gettagdescription' :
            	...
            	//parameter[1] = tag
            	//parameter[2] = precisa documentação
           	
            	//TODO documentar o porque desta comparação
                if($f->parameter[2] == 1)
                {
                ...
                $contentX = $this->busMaterial->getContentTag($f->parameter[0], $f->parameter[1], $this->line);
