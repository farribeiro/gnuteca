...
class GDependencyCheck extends BDependencyCheck
{
    public function listDependency()
    {
        ...
        $list[] = array('urlOfGnuteca', _M('URL do Gnuteca', $module));
    ...
    public function gCron()
    {
        ...
        if ( strlen($content) > 0 )
        {
            ...
            //se faz mais de 6 minutos desde a última execução, o gcron está parado
            if ( $diff->minutes <= 6)
            {
                $ok = true;
            }
    ...
    public function googleBook()
    {
        ...
        if ( !$allow )
        {
            $this->setMessage( _M("È necessário ativar a propriedade allow_url_fopen no php.ini") );
            return false;
        }
    ...
    public function urlOfGnuteca()
    {
        $prefix = $_SERVER['HTTPS'] ? 'https://' : 'http://'; //obtém flag https
        $apacheHost = $prefix . $_SERVER['HTTP_HOST'] . '/'; //obtém host do apache
        
        if ( $apacheHost != URL_GNUTECA )
        {
            $this->setMessage( _M('A URL da preferência "URL_GNUTECA" está configurada como "@1" e deve ser "@2"' ,'gnuteca3', URL_GNUTECA, $apacheHost) );
