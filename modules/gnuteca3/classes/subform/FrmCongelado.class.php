...
class FrmCongelado extends GSubForm
{
    ...
    public function createFields()
    {
        ...
        $fields[] = new GPersonLookup('personId', _M('Professor(a)', $this->modules), 'person');
    ...
    {
        $renewDate = $this->busRequestChangeExemplaryStatus->finalDate();
        $date      = new GDate($renewDate[1]);
        $fields[]  = $finalDate = new MCalendarField('finalDate', $date->getDate(GDate::MASK_DATE_USER), _M('Data final', $this->module), FIELD_DATE_SIZE);

        if ( REQUEST_CHANGE_EXEMPLARY_STATUS_BY_SEMESTER == DB_TRUE )
        {
    ...
    public function cancelReserve($args)
    {
        ...
        if ( $ok )
        {
            ...
            $mail->sendMailToAdminCancelRequestChangeExemplaryStatus($args->requestChangeExemplaryStatusId);
