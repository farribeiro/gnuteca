...
class GnutecaReceipt extends GMessages
{
    ...
    public function resendStoredMails()
    {
        ...
        if ( is_array($emails) )
        {
            foreach ( $emails as $line => $email )
            {
                if ( $email->getError() )
                {
                    $this->addError( _M('Problemas ao reenviar email para <b>@1</b>.Mensagem do servidor: @2',MIOLO::getCurrentModule(),$email->getEmail(),$email->getError()) );
                }
                else
                {
    ...
    /**
     * Envia o recibo para o servidor de impressora ou para o browse
    ...
    public function sendPrintServer($receiptText)
    {
        ...
        if ( PRINT_MODE == self::PRINT_MODE_SOCKET ) //Envia o conteudo do recibo para a impressora
        {
        ...
        elseif ( PRINT_MODE == self::PRINT_MODE_NAVIGATOR ) //Imprimi pelo navegador
        {
            return BusinessGnuteca3BusFile::openDownload('receipt',"receipt.".mktime()+rand().'.rcpt', $receiptText, 'recibo.rcpt');
        }
        else
        {
            return false;
