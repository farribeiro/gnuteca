...
class FrmRequestChangeExemplaryStatus extends GForm
{
    ...
    {
        $fields[] = new GSelection('statusId',  $this->statusId->value, _M('Estado', $this->module), $this->busStatus->listRequestChangeExemplaryStatusStatus($this->statusId->value), NULL, NULL, NULL, TRUE);
        $fields[] = new MTextField("requestChangeExemplaryStatusId",   $this->requestChangeExemplaryStatusId->value, _M("Código",          $this->module), FIELD_ID_SIZE);
        ...
        $fields[] = $personId = new GPersonLookup('personId', _M('Pessoa', $this->modules), 'activePerson');

        // EXEMPLARY STATUS SELECT
        $fields[] = new GSelection('exemplaryStatusId', $this->exemplaryStatusId->value, _M('Estado futuro', $this->module), $this->busExemplaryStatus->listExemplaryStatus(null, true));
        ...
        $fields[] = new MCalendarField('date',         $this->date->value,        _M('Data',          $this->module), FIELD_DATE_SIZE);
        $fields[] = new MCalendarField('finalDate',    $this->finalDate->value,   _M('Data final',    $this->module), FIELD_DATE_SIZE);
    ...
    private function setFormOptions()
    {
        ...
        {
            $this->statusId->setReadOnly(true);
            $this->statusId->setValue(REQUEST_CHANGE_EXEMPLARY_STATUS_REQUESTED);
        }
    ...
    public function tbBtnSave_click($sender)
    {
        ...
        if(is_array($composition))
        {
            ...
            {
            	//Não adicionar exemplares removidos
    ...
    private function insertRequest($formData, $composition)
    {
        ...
        $this->busOperation->setFutureStatusId  ($formData->exemplaryStatusId);
    ...
    private function updateRequest($formData, $composition)
    {
        ...
        $this->busOperation->setRequestChangeExemplaryStatusStatusId($formData->statusId);
        $this->busOperation->setLibraryUnit                         ($formData->libraryUnitId);
        $this->busOperation->setPersonId                            ($formData->personId);
        $this->busOperation->setDate                                ($formData->date);
        $this->busOperation->setFinalDate                           ($formData->finalDate);
