    ...
    */
    public function onkeydown119( $args = NULL )
    {
        ...
        if (GPerms::checkAccess('gtcMaterialMovementRequestReserve', NULL, false) || GPerms::checkAccess('gtcMaterialMovementAnswerReserve', NULL, false))
        {
            ...
            $optionCont[0] = new GRadioButtonGroup('option',_M('Tipo', $module), array(_M('[F5] Obra', $module), _M('[F6] Exemplar',$module) ), '[F6] Exemplary', null, 'horizontal');
            ...
            $tableItemsReserve  = new GRepetitiveField('tableItemsReserve', _M('Itens', $module), null, null, array('noButtons'));
    ...
    public function addReserve($args)
    {
        ...
        if ($this->busOperationReserve->getErrors() )
        {
            $this->busOperationReserve->unsetBlockReserveProcess();
            $table = $this->busOperationReserve->getMessagesTableRaw() ;
        ...
        $gotoYes = "javascript:miolo.getElementById('initialStatusConfirmed').value = 'yes';" . $goto;
    ...
    {
    	$this->jsDisabled('libraryUnitId', false);
    ...
    {
        GRepetitiveField::setData($items,'tablePolicyReserve');
    ...
    public function onkeydown115_119( $args )
    {
        ...
        $this->injectContent($table, "dojo.byId('personId').focus();", true);
        $this->cleanData($args);
    }
