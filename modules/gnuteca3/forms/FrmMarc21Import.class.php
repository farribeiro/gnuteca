...
class FrmMarc21Import extends GForm
{
    ...
    {
        $module = $this->module;
        $fields[] = new MTextField('fieldDelimiter','\n', _M('Limitador de campo', $module),FIELD_DESCRIPTION_SIZE, _M('Caractere que separa os campos', $this->module));
        $fields[] = new MTextField('subFieldDelimiter','|', _M('Limitador de subcampo', $module),FIELD_DESCRIPTION_SIZE, _M('Caractere que separa os subcampos. Valores comuns "$", "^" e "|"'));
        $fields[] = new MTextField('emptyIndicator','#', _M('Indicador vazio', $module),FIELD_DESCRIPTION_SIZE, _M('Caractere que indica quando o indicador está vazio', $this->module));
        $fields[] = new MTextField('recordDelimiter','---', _M('Limitador de registro', $module),FIELD_DESCRIPTION_SIZE);
        $fields[] = new GFileUploader('Envie seu arquivo');
        GFileUploader::setExtensions(array('txt')) ;
    ...
    public function tbBtnSave_click($sender=NULL, $confirm = DB_FALSE)
    {
        ...
        GFileUploader::clearData();//após pegar limpa para evitar reuso
