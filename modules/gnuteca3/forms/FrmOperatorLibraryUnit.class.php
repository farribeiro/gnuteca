...
class FrmOperatorLibraryUnit extends GForm
{
    ...
    public function saveUser($data = null)
    {
        ...
        {
            //Faz um foreach no groups e ver o que está deletado no $data e deletar no $oldgroups
            foreach ( $data->group as $dataGroup )
            {
                if ( $dataGroup->removeData )
                {
                    foreach ( $oldGroups as $index => $oldGroupId ) //Percorre todos os $oldGroups
                    {
                        if ( $oldGroupId == $dataGroup->groupId ) //Se o id do oldGroup for igual ao Id do Grupo que deve ser excluído
                        {
                            unset ($oldGroups[$index]); //Retira do array para não salvar um grupo que foi removido da GRepetitiveField
                        }
                    }
                }
            }

            $newGroups = array_unique(array_merge($groups, $oldGroups)); //Mescla $groups e $oldGroups retirando valores duplicados.
        }
    ...
    public function tbBtnSave_click($sender = null)
    {
        ...
        if ( $this->function != 'update' )
        {
            //verifica se ja existe algum registro para esse operador na gtclibraryunit
            $dataOperator = $this->business->getOperatorLibraryUnit($data->operator);

            if ( is_array($dataOperator->operatorLibrary) )
            {
                $this->error(_M('Já existe um registro para este operador', $this->module));
                return;
            }
        }
    ...
    public function loadFields()
    {
        ...
        if ( MIOLO::_REQUEST('libraryUnits') != DB_FALSE )
        {
            ...
            if ($this->business->operatorLibrary[0]->libraryUnitId)
            {
