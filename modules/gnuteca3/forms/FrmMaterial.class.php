...
class FrmMaterial extends GForm
{
    ...
    public $business;
    public  $businessMaterialControl,
            $busMaterialHistory,
            $businessTag,
            $localFields,
            $grid,
    ...
    public  $imagePlusUrl   = null,
            $imageMinusUrl  = null;
    
    private $busMarcTagListing,
            $busPrefixSuffix,
            $busSeparator;
    ...
    function __construct($data)
    {
        ...
        switch($this->function)
        {
            ...
            case 'duplicate':
                ...
                {
                    $this->business->setDuplicateMaterial();
                    $this->business->controlNumber = $this->controlNumber;
                }
    ...
    function mainFields()
    {
        ...
        $fields[] = new MButton('btnNext', _M('Próximo', $this->module), ":createSpreadsheetFields", $this->ui->getImage($this->module, 'next-16x16.png') );
    ...
    {
        $this->imagePlusUrl     = $this->ui->getImage($this->module, 'plus-8x8.png');
        $this->imageMinusUrl    = $this->ui->getImage($this->module, 'minus-8x8.png');
    ...
    function getSpreadsheetFields()
    {
        ...
        foreach($this->spreadsheet as $tabName => $etiquetas)
        {
            ...
            foreach($etiquetas as $etiquetaId => $etiContent)
            {
                ...
                if($permiteRepetitiveField)
                {
                    ...
                    $this->repetitiveFields[$etiquetaId] = new GRepetitiveField(self::REPEAT_FIELD_PREFIX_NAME. $etiquetaId, $etiContent->name, null, true, true, 'vertical');
                    ...
                    if($this->function != 'insert' && count($etiContent->loadValue))
                    {
                        $displayGroupForce = true;
                        $this->repetitiveFields[$etiquetaId]->setData($etiContent->loadValue);
                    }
        ...
        $GFileUploader = new GFileUploader('Anexe a capa do material');
        GFileUploader::setLimit(1); //somente uma imagem por capa
    ...
    public function getCoverFromGoogleConfirm( $args )
    {
        ...
        GFileUploader::setData( array($file ) );
        GFileUploader::generateTable(); //atualiza a tabela
    }
    ...
    public function saveHelp( )
    {
        ...
        {
            $urlHelpImage = GUtil::getImageTheme('helpAdd-16x16');
    ...
    public function getHelpField($name, $tag, $hasHelp )
    {
        ...
        if ( $this->canEditHelp )
        {
            $imageHelp = $hasHelp ? GUtil::getImageTheme('help-16x16') : GUtil::getImageTheme('helpAdd-16x16');
        }
        //caso não tenha permissão de edição e o campo tenha ajuda
        else
        {
            $imageHelp = $hasHelp ? GUtil::getImageTheme('help-16x16') : null;
        }
    ...
    private function getCatalogueField($object, $value = '', $readOnly = false )
    {
        switch($object->type)
        {
            ...
            case MARC_PERIODIC_INFORMATIONS:
                ...
                $field1 = new GContainer(null, array(new MDiv(null, $lv), new MTextField("especialField_". MARC_PERIODIC_INFORMATIONS ."_{$object->fieldName}[]", $v, '',  10)));
                $field2 = new GContainer(null, array(new MDiv(null,$ln), new MTextField("especialField_". MARC_PERIODIC_INFORMATIONS ."_{$object->fieldName}[]", $n, '',  10)));
                $field3 = new GContainer(null, array(new MDiv(null, _M('ano', $this->module)), new MTextField("especialField_". MARC_PERIODIC_INFORMATIONS ."_{$object->fieldName}[]", $pt, '', 15)));

                return new GContainer("lookUpContainer_{$object->fieldName}", array($field1, $field2, $field3, $field4));
     ...
     */
    public function saveCover($controlNumber, $folder =  'cover')
    {
        $busFile        = $this->MIOLO->getBusiness('gnuteca3','BusFile');
        ...
        {
            $ori = $busFile->getAbsoluteFilePath( 'coverpre', $controlNumberPost, 'png');
            $dst = $busFile->getAbsoluteFilePath( 'cover', $busCatalogue->getControlNumberFromSession() , 'png');
        ...
        if ( $coverData )
        {
            foreach ( $coverData as $i => $file )
            {
                ...
                if ( $file->removeData )
                {
                    $filePath = $busFile->getAbsoluteFilePath( $folder, $controlNumber, 'png');
                ...
                else
                {
                    ...
                    if ( $file->insertData )
                    {
                        $file->basename = $controlNumber.'.png';
    ...
    function saveSpreadsheet($args)
    {
        ...
        if ( GPerms::checkAccess('gtcMaterial', $function, false) )
        {
            ...
            if ( $this->function != 'update' ) // Se for uma inserção de material
            {
                $this->jsSetValue( 'entryDate', GDate::now() ); // Seta data atual por javascript no campo entryDate.
            }
    ...
    public function getSaveContent( $preCatalogue = false )
    {
        ...
        if ($menu)
        {
            ...
            $buttons['newChildren'] = new GRealLinkButton('newChildren', $menu->menuname, "main:catalogue:material&function=addChildren&controlNumber={$controlNumber}&leaderString=$menuoption", GUtil::getImageTheme('addChild-16x16.png'));
        }
