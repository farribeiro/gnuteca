...
class FrmLoan extends GForm
{
    ...
    public function mainFields()
    {
        ...
        $fields[] = new MCalendarField('loanDate', GDate::now(), _M('Data', $this->module), FIELD_DATE_SIZE );
        ...
        if ($data->libraryUnitId)
        {
            ...
            if ($data->libraryUnitId && $data->linkId && $data->materialGenderId)
            {
                ...
                if ($policy->privilegeGroupId) //objeto retornado existe
                {
                    ...
                    $policy->forecastDate = $forecastDate->getDate(GDate::MASK_DATE_USER);

                    $this->page->addJsCode("
                    document.getElementById('returnForecastDate').value = '{$policy->forecastDate}';
                    dojo.byId('widget_returnForecastDate').childNodes[0].childNodes[2].childNodes[1].value = '{$policy->forecastDate}';  //POG?
