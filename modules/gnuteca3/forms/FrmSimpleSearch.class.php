...
class FrmSimpleSearch extends GForm
{
    ...
    public function defineFields( $args , $subForm)
    {
    ...
    public function btnLoginUpper_click( $args, $subForm=false )
    {
        ...
        {
            $person = $busPerson->getPersonIdFormLoginAndBase($args->personIdUpper, $args->baseLdap); //obtém a pessoa

            //troca o login do usuário pelo personId
            if ( strlen($person->personId) > 0) //se tiver a pessoa na base, troca o login pelo personId
            {
                $args->personIdUpper = $person->personId;
            }
            else //insere a nova pessoa, seta o personId
            {
                $busPerson->beginTransaction();
                $args->personIdUpper = $busPerson->insertLdapPerson($args->personIdUpper, $args->baseLdap, $args->passwordUpper);
                $busPerson->commitTransaction();
            }
        }
        
        //faz a autenticação
        $busAuthenticate->authenticate( $args->personIdUpper, $args->passwordUpper );
    ...
    public function getFields( $args , $returnFieldsToAjax = false, $subForm )
    {
        ...
        else
        {
            ...
            $libraryUnitId = new GSelection('libraryUnitId', $formContent['libraryUnitId'], null , $librarys , null,null, null, true );
        ...
        $accurateTerm->addAttribute('onclick', "gnutecaSearch.verifyCheckboxes(this.name)");
        ...
        if ($formContent['letterField'])
        {
            ...
            if ( is_array( $listMaterialType ) )
            {
            	...
            	{
            		if ( $materialType[0] == MATERIAL_TYPE_ID_PERIODIC_COLLECTION )
            		{
            			$materialTypeDescription = $materialType[1];
            		}
            	}
            }

            $listMaterialType = array( MATERIAL_TYPE_ID_PERIODIC_COLLECTION => $materialTypeDescription );
        }
        else
        {
        ...
        {
            $listTermType[$line] = _M( 'Filtrar por ','gnuteca3') . $term;
            break;
        }
    ...
    public function btnReport_click($sender, $forSendEmail = FALSE)
    {
		...
		if ($forSendEmail)
		{
			return BusinessGnuteca3BusFile::getAbsoluteFilePath('pdf',$report->filename);;
		}
    }
    ...
    public function btnSearch_click($args)
    {
        ...
        if( !GOperator::isLogged() && defined("SIMPLE_SEARCH_EXCLUDE_EXEMPLARY_STATUS") )
        {
             $busGenericSearch->addExemplaryControlWhere("exemplaryStatusId", explode(",", SIMPLE_SEARCH_EXCLUDE_EXEMPLARY_STATUS    ), "AND", "NOT IN");
        }
        ...
        if (is_array($args->termText))
        {
            ...
            foreach ($args->termText as $line=> $info)
            {
                ...
                if ($info && $termType)
                {
                    $operation = $args->termOpt[$line+1]; //precisa do +1 em função de algumas contagens estranhas na criação do campo
                ...
                else if ($info)  //se vier expressão
                {
                    ...
                    $busGenericSearch->addMaterialWhereByExpression($exp);
                }
            }
            ...
            if ($exp || $ok)
            {
                ...
                if ($args->libraryUnitId)
                {
                     ...
                     $busGenericSearch->addExemplaryControlWhere( array('libraryUnitId', 'originalLibraryUnitId'), $args->libraryUnitId, "AND", "IN");
                }
                ...
                //trabalha o filtro de aquisição
                if (( $args->aquisitionFrom) && $args->aquisitionTo)
                {
                    $busGenericSearch->addMaterialControlWhere('entrancedate', array($args->aquisitionFrom, $args->aquisitionTo), 'AND', 'BETWEEN');
                }
                else
                if ($args->aquisitionFrom)
                {
                    $busGenericSearch->addMaterialControlWhere('entrancedate', array($args->aquisitionFrom),   'AND','>=');
                }
                else
                if ($args->aquisitionTo)
                {
                    $busGenericSearch->addMaterialControlWhere('entrancedate', array($args->aquisitionTo),   'AND','<=');
                }
