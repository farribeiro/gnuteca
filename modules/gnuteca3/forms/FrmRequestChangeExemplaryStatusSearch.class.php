...
class FrmRequestChangeExemplaryStatusSearch extends GForm
{
    ...
    public function mainFields()
    {
        ...
        $fields[] = $personId = new GPersonLookup('personIdS', _M('Pessoa', $this->modules), 'activePerson');
        
        $options        = $this->busExemplaryStatus->listExemplaryStatus(false, false, true);
        $fields[]       = new GSelection('futureStatusIdS', $args->futureStatusIdS, _M('Estado futuro', $this->module) , $options, null, null, null, false);
    ...
    public function aproveRequest( $args )
    {
        ...
        $fields[]   = new MTextField( null, $request->futureStatusId . ' - ' .$request->futureStatusDesc , _M('Estado futuro'),40, null,null, true);
        ...
        if ( is_array( $request->requestChangeExemplaryStatusComposition ) )
        {
            foreach ( $request->requestChangeExemplaryStatusComposition as $line => $composition )
            {
                ...
                $tableData[$line][4]  = $exemplary->title . ' - ' . $exemplary->author;
