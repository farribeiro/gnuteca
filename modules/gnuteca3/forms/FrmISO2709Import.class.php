...
{
    public $busMaterial, $busPreCatalogue;
    ...
    public function mainFields()
    {
        ...
        $fields[] = new GFileUploader( _M('Envie seu arquivo', $this->module) );
        GFileUploader::setExtensions(array('iso'), $deny) ;
        GFileUploader::setLimit(10);
        ...
        $this->_toolBar->disableButton( array('tbBtnSearch', 'tbBtnReset', 'btnFormContent'));

        $validators[] = new MRequiredValidator('recordDelimiter');
        $validators[] = new MRequiredValidator('fieldDelimiter');
        $validators[] = new MRequiredValidator('subFieldDelimiter');
    ...
    public function tbBtnSave_click($sender=NULL, $confirm = DB_FALSE)
    {
        ...
        $data->files = GFileUploader::getData();
        ...
        if ( $readRecords > 0 )
        {
            ...
            $this->question(_M('Tem certeza que deseja importar @1 registros?', $this->module, $readRecords), GUtil::getAjax('saveConfirmation'));
        }
        else
        {
            $this->information( _M('Não há registros para importar.', $this->module) );
        }
    }
    ...
    public function saveConfirmation()
    {
        ...
        GFileUploader::clearData(); //limpa a repetitive de arquivos
