...
function selectedItem(name,cont)
{
    ...
    if ( cont > 0 )
    {
        ...
        div.scrollTop = value *  ( firstItem.clientHeight + 10 ) ;
...
function onkeyUpDictionary( event, element , name, related, item, filter)
{
    ...
    else
    {
        ...
        {
            clearTimeout(t);
            t = setTimeout( "dictionaryAjax('"+element.id+"', '"+name+"', '"+related+"', '"+item+"', '"+filter+"');",500);
        }
        else
        {
            div.style.display='none';
...
function onkeyUpResponse(name)
{
   ...
   if ( isRepetivive && myElement)
   {
       ...
       myDiv.style.top = myElement.offsetTop-6+'px';
   }
}
