...
dojo.declare ("Gnuteca", null,
{
	...
	constructor: function()
	{
        ...
        dojo.connect( window, 'onclick', this.onclick);
	}
    ...
    /**
     * Função chamada no onclick do documento
     * Detecta define elemento ativo e desativa validadores em campo de seleção
     */
    onclick : function (e)
    {
        try
        {
            ...
            //FIXME aqui não entra no chrome 13
            //caso esteja clickando em um select ou input marca como modificado
    ...
    */
    ...
    help : function()
    {
        focus = document.activeElement; //obtém elemento em foco
    	miolo.doAjax('help',focus.id,frm);
    }
    ...
    setValueForRepetitive : function ( fieldId, value )
    {
        ...
        //retira barra extra adiciona pelo php
        value = value.replace('//','/','g');//g troca todos
    ...
    {
        item = dojo.byId( starName + 'star' + selectedItem) ;
...
dojo.declare ("GnutecaSearch", null,
{
    ...
    //Impede que 2 checkboxes sejam checkados simultaneamente
    verifyCheckboxes : function(cbId)
    {
        if (!document.getElementById(cbId).checked)
        {
            return;
        }
        if (cbId == 'accurateTerm' && document.getElementById('termCrossing').checked)
        {
            document.getElementById('termCrossing').checked = false;
        }
        if (cbId == 'termCrossing' && document.getElementById('accurateTerm').checked)
        {
            document.getElementById('accurateTerm').checked = false;
        }
    },
