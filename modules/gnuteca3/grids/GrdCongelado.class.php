...
class GrdCongelado extends GGrid
{
    ...
    function __construct($data)
    {
        ...
        (
            ...
            new MGridColumn( _M('Data',             $this->module), MGrid::ALIGN_CENTER,   null, null, $view, null, true ),
            new MGridColumn( _M('Data final',       $this->module), MGrid::ALIGN_CENTER,   null, null, $view, null, true ),
    ...
    public function checkValues($i, $row, $actions, $columns)
    {
    	...
    	if ( $userCode != $personId || $requestStatusId == REQUEST_CHANGE_EXEMPLARY_STATUS_CANCEL ||$requestStatusId == REQUEST_CHANGE_EXEMPLARY_STATUS_CONCLUDE || $requestStatusId == REQUEST_CHANGE_EXEMPLARY_STATUS_REPROVED)
    	{
            $this->page->onLoad("dojo.byId('selectGrdCongelado[{$i}]').disabled = 'disabled';
                                 dojo.byId('selectGrdCongelado[{$i}]').style.display = 'none';");
            $actions[0]->disable();
        }
        elseif($userCode == $personId)
        {
            $actions[0]->enable();
        }
        ...
        $renew      = $this->busRequestChangeExemplaryStatus->finalDate();
