...
class GrdLoanBetweenLibrary extends GSearchGrid
{
    ...
    public function __construct($data)
    {
        ...
        $columns = array(
            ...
            new MGridColumn(_M('Data do empréstimo', $this->module),                 MGrid::ALIGN_RIGHT,  null, null, false, null, true),
            new MGridColumn(_M('Data prevista da devolução', $this->module),      MGrid::ALIGN_RIGHT,  null, null, false, null, true),
            new MGridColumn(_M('Data de devolução', $this->module),               MGrid::ALIGN_RIGHT,  null, null, false, null, true),
            new MGridColumn(_M('Data limite', $this->module),                MGrid::ALIGN_RIGHT,  null, null, false, null, true),
    ...
    public function checkValues($i, $row, $actions, $columns)
    {
        ...
        if (GPerms::checkAccess($this->transaction, 'update', false))
        {
            ($columns[11]->control[$i]->value == DB_TRUE) ? $actionCancel->show() : $actionCancel->hide();
            ($columns[13]->control[$i]->value == DB_TRUE) ? $actionConfirmReceipt->show() : $actionConfirmReceipt->hide();
            ($columns[14]->control[$i]->value == DB_TRUE) ? $actionReturnMaterial->show() : $actionReturnMaterial->hide();
            ($columns[15]->control[$i]->value == DB_TRUE) ? $actionConfirmReturn->show() : $actionConfirmReturn->hide();
            ($columns[16]->control[$i]->value == DB_TRUE) ? $actionUpdate->enable() : $actionUpdate->disable();
            ($columns[16]->control[$i]->value == DB_TRUE) ? $actionDelete->enable() : $actionDelete->disable();

            if ( !$actionDelete->enabled ) //Se a ação de delete do registro estiver habilitada
            {
                //desabilita o checkbox do registro.
                $this->page->onLoad("dojo.byId('selectGrdLoanBetweenLibrary[{$i}]').disabled = 'disabled';
                                     dojo.byId('selectGrdLoanBetweenLibrary[{$i}]').style.display = 'none';");
            }
