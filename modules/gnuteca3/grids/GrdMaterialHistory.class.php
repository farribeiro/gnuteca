...
class GrdMaterialHistory extends GGrid
{
    ...
    public function __construct($data)
    {
        ...
        $columns = array(
            ...
            new MGridColumn(_M('Data/Hora', $this->module), MGrid::ALIGN_LEFT,  null, null, true, null, true),
    ...
    public function checkValues($i, $row, $actions, $columns)
    {
        ...
        //Conteudo anterior
        if ( strlen($columns[7]->control[$i]->value) > 0 )
        {
            $newContent = $this->busMaterial->optionsTable($tag, $columns[7]->control[$i]->value);
            if ($newContent)
            {
                $columns[7]->control[$i]->value .= ' - ' . $newContent ;
            }
        }
        //conteudo atual
