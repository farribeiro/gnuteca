...
class GrdSimpleSearch extends GAddChildGrid
{
    ...
    public function actionIcons()
    {
        ...
        if ( $this->externalSearch() )
        {
            ...
            $this->actionGetControlNumber = $this->addActionIcon( _M('Selecionar', $this->module), GUtil::getImageTheme('getControlNumber-28x28.png') , "javascript:document.domain = 'univates.br'; this.opener.document.getElementById('{$element}').value = %0%; try{this.opener.document.getElementById('{$element}').onblur();}catch(e){} window.close();");
        	return;
        }
        ...
        if ( is_array($exemplarys) && ($exemplarys) )
        {
            ...
            foreach ($exemplarys as $libraryUnitId => $exemplaryStatus) //libraryUnitId
            {
                ...
                else
                {
                    ...
                    if ( ( (!MIOLO::_REQUEST('libraryUnitId','POST')) || (strpos(MIOLO::_REQUEST('libraryUnitId','POST'), ","))  ) && $_SESSION['libraryCount'] > 2 || $emprestadoPara )
                    {
                        $lbl = new MLabel( $emprestadoPara . $exemplaryData[$libraryUnitId]->title  );
                        $lbl->setBold(TRUE);
