...
class BusinessGnuteca3BusExemplaryStatusHistory extends GBusiness
{
    ...
    public function searchExemplaryStatusHistory()
    {
        ...
        $columns = 'D.controlNumber,
                    A.itemNumber,
                     ( SELECT content FROM gtcmaterial WHERE fieldid = \''.$titleTag[0].'\' and subfieldid = \''.$titleTag[1].'\' and controlnumber = d.controlNumber ) AS title,
                    ( SELECT content FROM gtcmaterial WHERE fieldid = \''.$authorTag[0].'\' and subfieldid = \''.$authorTag[1].'\' and controlnumber = d.controlNumber ) AS author,
                    B.description,
                    C.libraryName,
                    A.date,
                    A.operator';
