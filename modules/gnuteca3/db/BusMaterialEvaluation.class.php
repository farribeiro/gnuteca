...
class BusinessGnuteca3BusMaterialEvaluation extends GBusiness
{
    ...
    public function searchMaterialEvaluation($toObject = FALSE)
    {
        ...
        if ($this->dateS)
        {
            if ( $this->timeS )
            {
                $this->setWhere( "e.date::date = '{$this->dateS}'::date" ); //Onde data for tal
                $this->setWhere( "substring(e.date::time || '' from 1 for 5)::time = substring('{$this->timeS}'::time || '' from 1 for 5)::time" ); //e hh:mm tal
            }
            else
            {
                $this->setWhere( "e.date::date = '{$this->dateS}'::date " );
            }
        ...
        $this->tables = 'gtcMaterialEvaluation e LEFT JOIN basPerson p ON ( e.personId = p.personId) ';
    ...
    public function listEvalutionWithComment( $controlNumber, $toObject = false )
    {
        ...
        $this->columns = 'materialEvaluationId,controlNumber,e.personId,p.name, date,comment,evaluation';
        $this->tables = 'gtcMaterialEvaluation e LEFT JOIN basPerson p ON ( p.personId = e.personId )';
