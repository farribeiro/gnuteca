...
class BusinessGnuteca3BusOperationRequestChangeExemplaryStatus extends GMessages
{
    ...
    public function checkComposition($composition = null)
    {
        ...
        foreach ($composition as $index => $v)
        {
            ...
            if ( $this->busExemplaryControl->checkExemplaryExists($v->itemNumber, $libraryUnitId) )
            {
    ...
    public function insertRequest()
    {
        ...
        if(!$ok)
        {
            throw new Exception( _M("Erro na inserção do pedido", $this->module) );
        }
    ...
    public function updateRequest()
    {
        ...
        $ok = $this->checkUpdateStatus($this->getStatus(), $this->busReqChanExeStatus->getCurrentStatus($this->requestChangeExemplaryStatusId));
    ...
    public function aproveRequest($requestId = null, $confirmItemNumber = null, $futureStatus = null, $checkUpdateStatus = true)
    {
        ...
        if(!strlen($requestId))
        {
            throw new Exception( _M("Esta requisição de alteração de estado é inválida!", $this->module) );
        }
        ...
        {
            $this->checkUpdateStatus(REQUEST_CHANGE_EXEMPLARY_STATUS_APROVED, $this->busReqChanExeStatus->getCurrentStatus($requestId));
        }
        ...
        {
            throw new Exception( _M("Nenhum item foi confirmado. A operação não pode ser aprovada.",   $this->module) );
        }
        ...
        {
            throw new Exception( _M("Não é possível aprovar a requisição!",  $this->module) );
        }
    ...
    private function appliedChangeStatusForExemplaries( $exemplaries, $requestId, $futureStatus, $aproveJustOne = false )
    {
        ...
        foreach ($exemplaries as $itemNumber)
        {
            ...
            if (!$agendar)
            {
                ...
                $this->addError( _M("Não foi possível definir um método para alterar o estado. Verificar se o estado do material é um estado inicial ou se este permite agendamento de troca de estado.",   $this->module) );
                continue;
            }
    ...
    public function reproveRequest($requestId = null, $checkUpdateStatus = true)
    {
        ...
        if(!strlen($requestId))
        {
            throw new Exception( _M("Esta requisição de alteração de estado é inválido!", $this->module) );
        }
        ...
        {
            $this->checkUpdateStatus(REQUEST_CHANGE_EXEMPLARY_STATUS_REPROVED, $this->busReqChanExeStatus->getCurrentStatus($requestId));
        }

        if(!$this->busReqChanExeStatus->changeStatusRequestChangeExemplaryStatus($this->getOperatorId()))
        {
            throw new Exception(_M("Não é possível reprovar a requisição!", $this->module) );
        }
    ...
    public function concludeRequest($requestId = null, $checkUpdateStatus = true)
    {
        ...
        if($checkUpdateStatus)
        {
            $this->checkUpdateStatus(REQUEST_CHANGE_EXEMPLARY_STATUS_CONCLUDE, $this->busReqChanExeStatus->getCurrentStatus($requestId));
        }

        if(!$this->busReqChanExeStatus->changeStatusRequestChangeExemplaryStatus($this->getOperatorId()))
        {
            throw new Exception( _M("Não é possível concluír a requisição!", $this->module));
        }
    ...
    public function cancelRequest($requestId = null, $checkUpdateStatus = true)
    {
        ...
        if($checkUpdateStatus)
        {
            $this->checkUpdateStatus(REQUEST_CHANGE_EXEMPLARY_STATUS_CANCEL, $this->busReqChanExeStatus->getCurrentStatus($requestId));
        }

        //tenta trocar o estado dos exemplares
        if(!$this->busReqChanExeStatus->changeStatusRequestChangeExemplaryStatus($this->getOperatorId()))
        {
            throw new Exception( _M("Não é possível cancelar a requisição!", $this->module) );
        }
    ...
    */
    ...
    {
        $extraErrorMessage = ' ' . implode(' ',$this->getErrors() );
        $this->checkComposition();

        //TODO trocar pro throw ne Exception
        if(!$this->getLibraryUnit())
        {
            throw new Exception( _M("Esta unidade de biblioteca é inválida!", $this->module) . $extraErrorMessage );
        }
        elseif(!$this->getPersonId())
        {
            throw new Exception( _M("Este código de pessoa é inválido!", $this->module) . $extraErrorMessage );
        }
        elseif(!$this->getPersonLink())
        {
            throw new Exception( _M("Este vínculo de pessoa é inválido!", $this->module) . $extraErrorMessage );
        }
        elseif(!$this->getFutureStatusId())
        {
            throw new Exception(_M("Este código do estado futuro do exemplar é inválido!", $this->module) . $extraErrorMessage);
        }
        elseif(!$this->getDate())
        {
            throw new Exception( _M("A data é inválida!", $this->module) . $extraErrorMessage);
        }
        elseif(!$this->getFinalDate())
        {
            throw new Exception( _M("Esta data final é inválida!",$this->module) . $extraErrorMessage);
        }
        elseif(!$this->getRequestComposition())
        {
            throw new Exception(  _M("É necessário selecionar exemplares válidos para efetuar a requisição!", $this->module) . $extraErrorMessage);
        }
        elseif(!$this->getDiscipline())
        {
            throw new Exception(  _M("É necessário preencher a disciplina", $this->module) . $extraErrorMessage );
        }
    ...
    {
        //caso seja o mesmo estado não ativa alteração
        ...
        if ( !$result )
        {
  //          throw new Exception(_M("Não é permitido mudar uma requisição com estado @1 para o estado @2",   $this->module, $this->getLabelOfStatus( $this->busReqChanExeStatus->getCurrentStatus($this->requestChangeExemplaryStatusId) ) , $this->getLabelOfStatus( $this->getStatus() ) .'.') );
        }
