...
class BusinessGnuteca3BusOperationReturn extends GMessages
{
    ...
    public function addItemNumber( $itemNumber )
    {
    	$MIOLO  = MIOLO::getInstance();
    	$module = MIOLO::getCurrentModule();
        ...
        if ( $exemplary->exemplaryStatusId != DEFAULT_EXEMPLARY_STATUS_DISPONIVEL )
        {
            ...
            $exemplary->printReceiptLabel = MUtil::getBooleanValue( $exemplary->printReceipt) ? $imgPrint : '';
        }
    ...
    public function finalize( $data = null , $generateReceipt = true )
    {
        ...
        if ( is_array( $items) )
        {
            ...
            foreach ( $items as $line => $info )
            {
                ...
                // se o isReserveStatus do estado atual for TRUE desconsidera o agendamento
                if ( $futureStatusTMP && MUtil::getBooleanValue( $isReserveStatus ) )
                {
                ...
                if ( $info->loan )
                {
                    ...
                    if ( $info->loan->delayHours > 0 || $info->loan->delayDays > 0 )
                    {
                        ...
                        if ( $fineValue > 0 )
                        {
                            ...
                            $fineData->fineStatusId  = ID_FINESTATUS_OPEN;
                            ...
                            {
                                $defaultFineStatus = $this->busFineStatus->getFineStatus(ID_FINESTATUS_OPEN);
                            }
                ...
                if (MUtil::getBooleanValue($exemplaryStatus->meetReserve) == TRUE && $info->hasReserve)
                {
     ...
     *
     */
    private function confirmRequestChangeStatusExemplary($itemNumber, $exemplaryFutureStatusDefinedId = null)
    {
