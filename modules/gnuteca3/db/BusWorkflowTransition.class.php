...
class BusinessGnuteca3BusWorkflowTransition extends GBusiness
{
    ...
    function searchWorkflowTransition( $toObject = false , $removeInitialTransation = false )
    {
        ...
        $this->setColumns('WT.previousWorkflowStatusId,WT.nextWorkflowStatusId,(SELECT distinct(label) from gtcdomain WHERE key = prevStatus.workflowid LIMIT 1) as workflow,prevStatus.name,nextStatus.name,WT.function,WT.name,nextStatus.transaction as nextTransaction');
