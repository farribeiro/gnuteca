...
class BusinessGnuteca3BusOperationLoan extends GOperation
{
    ...
    public function setPerson($personId)
    {
        ...
        $person = $this->busPerson->getPerson( $personId , true, true); //pega somente link ativo
        ...
        if (!$person->personId)
        {
            $this->addError( _M('Pessoa não encontrada', $this->module), $personMSG);
            return false;
        }
        ...
        if (!$link)
        {
            $this->addError( _M('Usuário não possui vínculo.', $this->module), $personMSG );
            return false;
        }
        ...
        if (!$access)
        {
            $this->addError( _M('Usuário não tem acesso a biblioteca @1.', $this->module, $libraryUnit->libraryName), $personMSG );
            return false;
        }
        ...
        if (!$policy)
        {
            $this->addError( _M('Não existem políticas para o grupo @1.', $this->module, $link->description), $personMSG );
            return false;
        }
        ...
        for ( $i=0; $i < count( $policy ); $i++ )
        {
            ...
            foreach ($loans as $loan)
            {
                if ($policy[$i]->materialGenderId == $this->busExemplaryControl->getMaterialGender($loan->itemNumber))
                {
                    ...
                    if ( $loan->loanTypeId == ID_LOANTYPE_DEFAULT )
                    {
     ...
     * @return unknown
     */
    public function addItemNumber( $itemNumber, $loanTypeId )
    {
        $this->fine = null;
        ...
        if ( is_array( $loanToday) && is_array($person->policy))
        {
        	...
        	{
        		if ($info->privilegeGroupId == $person->privilegeGroupId)
        		{
        			$selectPolicy = $info;
        		}
        	}
        ...
        $exemplary->sendReceiptLabel  = MUtil::getBooleanValue($exemplary->sendReceipt) ? $imgEmail : '';
	        ...
	        foreach ($loans as $loan)
	        {
		        ...
		        if ( ($days == $config) && ($userBeforeExpired) )
		        {
                    ...
                    if(!strlen($loan->personEmail))
                    {
                        ...
                        $communicateMessage[$loan->loanId][3] = _M("E-mail pessoa está em branco", $this->module);
                        continue;
                    }
		        ...
		        if ( ($returnForecastDateObj->compare($beginDateObj, '>=')) && ($returnForecastDateObj->compare($endDateObj, '<=')) && ($userBeforeExpired) )
	            {
		                ...
		                && ($userBeforeExpired) ) //Preferencia de mandar email de aviso ativada
		            {
                        ...
                        if (!$loan->personEmail)
                        {
                            ...
                            $delayedLoanMessage[$loan->loanId][3] = _M("E-mail pessoa está em branco", $this->module);
                            continue;
                        }
