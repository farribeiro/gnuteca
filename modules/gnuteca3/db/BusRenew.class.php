...
class BusinessGnuteca3BusRenew extends GBusiness
{
    ...
    public function getRenewsOfLoan($loanId, $renewId=null)
    {
        ...
        $this->setColumns("A.renewid, 
                          ...
                          A.returnforecastdate, 
                          A.renewdate, 
                          (CASE WHEN newreturnforecastdate IS NOT NULL 
                          THEN 
                              A.newreturnforecastdate 
                          ELSE 
                              A.loanreturnforecastdate 
                          END) as newreturnforecastdate, 
                          operator");
        
        $tables = "(SELECT renewid, 
                                 loanid, 
                                 renewtypeid, 
                                 returnforecastdate, 
                                 renewdate, 
                                 (SELECT returnforecastdate 
                                   ...
                                   WHERE loanid = R.loanid 
                                     AND renewdate > R.renewdate 
                                ORDER BY renewdate ASC 
                                 LIMIT 1) as newReturnForecastDate, 
                          (SELECT returnforecastdate 
                             FROM gtcloan 
                            WHERE loanid = R.loanid) as loanreturnforecastdate, 
                                 operator 
                            FROM gtcrenew R 
                           WHERE R.loanid =  {$loanId}";
        if ( $renewId )
        {
            $tables .= " AND R.renewId = {$renewId}";
        }
        
        $tables .= "    ORDER BY renewdate ASC) A
                        LEFT JOIN  gtcRenewType B
                               ON  (A.renewTypeId = B.renewTypeId)";
