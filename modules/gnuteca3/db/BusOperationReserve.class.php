...
class BusinessGnuteca3BusOperationReserve extends GMessages
{
    ...
    */
    ...
    public function addItemNumber($itemNumber, $initialStatusConfirmed = NULL)
    {
        ...
        //TODO aqui pode ser otimizado para pegar somente loan para o itemNumber especifico
        if ($loans)
        {
            foreach ( $loans as $loan )
            {
                if ( $loan->controlNumber == $exemplary->controlNumber )
                {
                    $this->addError( _M('Usuário já possui empréstimo para este material.', $this->module), $reserveMSG);
                    $this->blockReserveProcess(true);
                    return false;
                }
            }
    ...
    function comunicateReserveAnswered()
    {
        ...
        foreach ($reserves as $line => $reserve)
        {
            ...
            if(!strlen($person->email))
            {
                ...
                $reserveMensage[$reserve->reserveId][3] = _M("E-mail pessoa está em branco", $this->module);
                continue;
            }
    ...
    */
    ...
    /**
     * Cancela todas as reservas para um itemNumber (atentididas e comunicadas)
     *
     * @param integer $itemNumber
     * @return boolean
     */
    public function cancelReserveByItemNumber($exemplary, $sendMailToUser = false)
    {
        $exemplaryReserves = $this->busReserve->getReservesOfExemplary($exemplary->itemNumber, $exemplary->exemplaryStatusId);
