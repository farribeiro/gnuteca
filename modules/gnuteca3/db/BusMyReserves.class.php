            ...
            foreach($result as $i => $v)
            {
                ...
                if($item)
                {
                    foreach($item as $i2 => $v2)
                    {
                        ...
                        //posição do usuário na reserva
                        $position = $this->businessReserve->getReservePosition($exemplary->itemNumber);
                        foreach ($position as $chave => $valor)
                        {
                            if ($valor[0] == $user)
                            {
                                $positionReserve = $chave+1 . ' º';
                            }
                        }
                    }
                }
