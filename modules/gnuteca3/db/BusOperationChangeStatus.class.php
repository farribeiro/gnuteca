...
class BusinessGnuteca3BusOperationChangeStatus extends GOperation
{
     ...
     * @return unknown
     */
    public function addItemNumber($itemNumber, $lowDate = null, $observation = null)
    {
        ...
        if ( !$itemNumber )
    	{
    		throw new Exception( _M('Número do exemplar não informado.', $this->module) );
    	}
        ...
        if (!$exemplary)
        {
            throw new Exception( _M('Exemplar não encontrado.', $this->module) );
        }
        ...
        if ( $this->getChangeType() == 2 )
        {
	        ...
	        {
	            throw new Exception( _M('Impossível adicionar exemplares em estado de baixa', $this->module) );
	        }

            //caso não possa o estado não permita agendar
            if ( ! MUtil::getBooleanValue( $exemplaryStatus->scheduleChangeStatusForRequest ) )
            {
                throw new Exception( _M('Estado "@1" não permite agendamento.', $this->module,$exemplaryStatus->description));
            }
        }
        ...
        {
            throw new Exception(_M('O item já tem agendamento de estado de baixa.', $this->module));
        }
        ...
        {
            throw new Exception(_M('É necessário definir a data de baixa.', $this->module));
        }
        ...
        {
            throw new Exception( _M('Exemplar não pertence a biblioteca.', $this->module));
        }
        ...
        if ( !isset($futureStatus) || ($futureStatus == 0) )
        {
            ...
            if ( !$futureStatus )
            {
                throw new Exception( _M('Estado futuro do exemplar não encontrado', $this->module) );
            }
        }

        if ( $exemplary->exemplaryStatusId == $futureStatus )
        {
            throw new Exception( _M('O estado futuro do exemplar é igual ao estado futuro atual.', $this->module) );
        }
        ...
        if ( ($this->getChangeType() == 2)  && ($status->level == ID_EXEMPLARYSTATUS_INITIAL))
        {
            throw new Exception( _M('Impossível agendar mudança de estado de materiais no nível inicial.', $this->module));
        }

        if ( ($this->getChangeType() != 2 ) )
        {
            if ( MUtil::getBooleanValue( $status->isReserveStatus ) )
            {
                ...
                if ($reserve)
                {
                    throw new Exception( _M('O estado está como reservado e tem reserva.', $this->module) );
                }
            }
            ...
            if ( $loan )
            {
                throw new Exception( _M('Exemplar com empréstimo em aberto', $this->module) );
            }
        }
        ...
        foreach ($items as $itemNumber => $exemplary)
        {
            ...
            if($this->busExemplaryControl->changeStatus( $itemNumber, $this->getExemplaryFutureStatus() , $this->getOperator() ))
            {
                ...
                if ( !MUtil::getBooleanValue($changedStatus->executeLoan ) )
                {
                    ...
                    $this->busOperationReserve->cancelReserveByItemNumber($exemplary, true);
                }
            }
            else
            {
