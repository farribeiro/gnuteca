...
class BusinessAdminUser extends MBusiness implements IUser
{
    ...
    public function insert()
    {
        ...
        $sql = "insert into miolo_user values('" . $this->idUser . "', '" . $this->login . "', '" . $this->fullname . "', '" . $this->nickname . "', '" . $this->password . "', '" . $this->hash . "', '')";
    ...
    public function update()
    {
        ...
        $db = $MIOLO->getDatabase( 'admin' );
        $this->hash = md5( $this->password );
        $sql = "update miolo_user set login = '" . $this->login . "', name = '" . $this->fullname . "', nickname = '" . $this->nickname . "', m_password = '" . $this->password . "', confirm_hash = '" . $this->hash . "' 
    ...
    public function save()
    {
        ...
        //somente troca a senha se alterou ou está inserindo
        $object = $MIOLO->getBusiness( $module, 'user' );
        if (!$object->getByLoginPass($this->login, $this->password)->login)
        {
            $this->hash = md5( $this->password );
    ...
    public function listByFilters($filters)
    {
        ...
        if ( $filters->login )
        {
            $criteria->addCriteria( 'login', 'ILIKE', "'{$filters->login}%'" );
        }
        
        if ( $filters->fullname )
        {
            $criteria->addCriteria( 'fullname', 'ILIKE', "'{$filters->fullname}%'" );
        }
        
        if ( $filters->nickname )
        {
            $criteria->addCriteria( 'nickname', 'ILIKE', "'{$filters->nickname}%'" );
        }
        return $criteria->retrieveAsQuery();
    }
